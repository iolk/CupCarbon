...
public class Functions {
    ...
    public static String check(String[] args) {
        ...
        if (!error1 && !error2 && !error3) {
            state = "[MALICIOUS " + id[0] + "]";
            malicious = 0;
        }
        if (error1 && !error2 && !error3) {
            state = "[MALICIOUS " + id[1] + "]";
            malicious = 1;
        }
        if (!error1 && error2 && !error3) {
            state = "[MALICIOUS " + id[2] + "]";
            malicious = 2;
        }
        if (error1 && error2 && !error3) {
            state = "[MALICIOUS " + id[3] + "]";
            malicious = 3;
        }
        if (!error1 && !error2 && error3) {
            state = "[MALICIOUS " + id[4] + "]";
            malicious = 4;
        }
    ...
    public static String hash(String[] args) throws Exception {
        ...
        SuperFastHash b = new SuperFastHash(args[0], Long.valueOf(args[1]), Integer.valueOf(args[2]),
    ...
    public static String kafkaProduce(String[] args) {
        String result = "ok";

        try {
            KafkaConnect.produce(args[0], args[1]);
        } catch (Exception e) {
            result = "error";
            e.printStackTrace();
        }
    ...
    public static String kafkaConsume(String[] args) {
        String result = "";

        try {
            result = KafkaConnect.consume(args[0]);
        } catch (Exception e) {
            result = "error";
            e.printStackTrace();
        }
